...
from ..domain.models.task_type import TaskType, TaskComplexity
...
class ClassificationRequest(BaseModel):
    ...
    context: Optional[dict[str, str]] = Field(None, description="Additional context for classification")
    files_changed: Optional[list[str]] = Field(None, description="List of file paths that will be changed")
...
class ClassificationResult(BaseModel):
    ...
    estimated_tokens: int = Field(..., gt=0, description="Estimated token count for execution")
    classifier_used: Optional[str] = Field(None, description="Which classifier was used (heuristic/ml/llm)")
